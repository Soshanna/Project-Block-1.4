...
Void Console_obj::onFocus( Dynamic _){
{
		...
		HX_STACK_LINE(163)
		bool tmp2 = (tmp1 == HX_HCSTRING("Using the console requires hscript - please run 'haxelib install hscript'.","\x1a","\x91","\xca","\x5f"));		HX_STACK_VAR(tmp2,"tmp2");
		HX_STACK_LINE(163)
		if ((tmp2)){
			HX_STACK_LINE(164)
			::openfl::_legacy::text::TextField tmp3 = this->input;		HX_STACK_VAR(tmp3,"tmp3");
...
Void Console_obj::onFocusLost( Dynamic _){
{
		...
		if ((tmp2)){
			...
			::openfl::_legacy::text::TextField tmp3 = this->input;		HX_STACK_VAR(tmp3,"tmp3");
			HX_STACK_LINE(188)
			tmp3->set_text(HX_HCSTRING("Using the console requires hscript - please run 'haxelib install hscript'.","\x1a","\x91","\xca","\x5f"));
		}
		HX_STACK_LINE(191)
		::flixel::_system::debug::completion::CompletionList tmp3 = this->completionList;		HX_STACK_VAR(tmp3,"tmp3");
		HX_STACK_LINE(191)
		tmp3->__Field(HX_HCSTRING("close","\xb8","\x17","\x63","\x48"), hx::paccDynamic )();
	}
return null();
}
...
{
		HX_STACK_FRAME("flixel.system.debug.console.Console","registerObject",0x0bbfe598,"flixel.system.debug.console.Console.registerObject","flixel/system/debug/console/Console.hx",296,0x367943c8)
		HX_STACK_THIS(this)
